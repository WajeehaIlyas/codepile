...
pub fn lex_with_regex(input: &str) -> Result<Vec<Token>, LexError> {
    ...
    let token_specification: Vec<(Regex, Option<fn(&str) -> Token>)> = vec![
        ...
        //Invalid identifiers (digit followed by letters/underscores)
        (Regex::new(r"\d+[A-Za-z_]\w*").unwrap(), Some(|s| Token::Invalid(s.to_string()))),

        // Literals
        (Regex::new(r"\d+\.\d+").unwrap(), Some(|s| Token::FloatLit(s.parse::<f64>().unwrap()))),
        (Regex::new(r"\d+").unwrap(), Some(|s| Token::IntLit(s.parse::<i64>().unwrap()))),

        // String literals with escapes
        (Regex::new(r#""(\\.|[^"\\])*""#).unwrap(), Some(|s| {
        ...
        (Regex::new(r"[\p{L}_\p{So}][\p{L}\p{N}_\p{So}]*").unwrap(), Some(|s| Token::Identifier(s.to_string()))),
...
fn compute_line_col(input: &str, index: usize) -> (usize, usize) {
    let mut line = 1;
    let mut col = 1;
